...
`knews`는 최신 검색 엔진(Tavily)과 웹 브라우저 자동화(Playwright) 기술을 결합하여, 누구나 손쉽게 구글 뉴스 스타일의 뉴스 데이터를 모을 수 있도록 설계된 도구입니다.

---

## 🌟 주요 특징

- **스마트 검색:** Tavily API를 사용하여 구글 뉴스처럼 최신 뉴스를 빠르게 찾습니다. (월 1,000회 무료)
- **강력한 본문 추출:** JTBC, 조선일보뿐 아니라 뉴스1, 매경, KBS, ETNews 등 국내 뉴스 페이지에서 Playwright 보강 추출을 자동 적용합니다.
...
- **학습자 친화적 CLI:** `knews collect` 별칭과 `--prefer-browser` 옵션으로 설명은 단순하게, 추출 품질은 높게 가져갈 수 있습니다.
...
```bash
uvx --from git+https://github.com/daniel8824/korean-news-collector knews doctor
```
...
복사한 키를 프로그램이 인식할 수 있도록 설정해야 합니다. 가장 추천하는 방법은 `knews init`으로 **'~/.env'** 파일을 만드는 것입니다.

```bash
knews init
```

그 다음 생성된 `~/.env` 파일을 열어서 아래 값을 넣으세요.

```bash
TAVILY_API_KEY=tvly-본인의키
```
...
# Excel로 저장
knews search "스타트업 뉴스" -o excel

# Markdown으로 저장 (옵시디언, 노션, 블로그용)
knews search "건강 정보" -o md -s healthy.md

# 브라우저 우선 모드로 더 깊게 추출
knews search "조선일보 반도체" --prefer-browser
```
...
# 매경/뉴스1처럼 본문이 짧게 잡히는 경우 브라우저 우선 사용
knews extract https://www.mk.co.kr/news/it/123456 --prefer-browser
```
...
## 🛠 지원하는 주요 뉴스 사이트
아래 사이트들은 자동으로 Playwright(브라우저 렌더링)를 우선 사용하거나, 결과가 짧을 때 자동 보강 추출합니다.
- **방송사:** JTBC, MBC, SBS, YTN, TV조선, 채널A, KBS
- **포털/신문:** 네이버뉴스, 다음뉴스, 조선일보, 중앙일보, 동아일보, 한겨레, 경향신문, 뉴스1, 매경, ETNews, Nocut, ZUM 등
- **경제/산업지:** 서울경제, 머니투데이, 헤럴드경제, 이데일리, 아시아경제, 파이낸셜뉴스, 디지털데일리, 아이뉴스24, 뉴스is, MBN 등
